...
You can shut down all messages produced by {\tt qqcat} by {\tt options("cat\_verbose" = FALSE)}. 
...
{\tt qq} as the function in {\tt GetoptLong} by:
...
extract the correct variable name while only extracts {\tt a[1}, finally it generates an error when executing {\tt a[1}. In such condition, you should use other styles
...
Since {\tt qq} takes value of the last variable in the chunk as final returning values, you need 
