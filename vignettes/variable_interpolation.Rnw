...
One feature of {\tt qqcat()} is you can set a global prefix to the messages by {\tt qq.options("cat\_prefix")}, 
...
From version 1.1.2, {\tt qq.options()} can work in a local mode in which
...
(E.g., in {\bf lattice}, there is also a {\tt qq()} function as well) and if so, you may enforce
{\tt qq()} in your working environment as the function in {\bf GetoptLong} by:
