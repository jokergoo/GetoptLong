...
replace with their values. The function is {\tt qq} which is named from the
...
By the way, another feature of {\tt qqcat} is you can set a global prefix to the message
printed by {\tt qqcat}. It can be set through {\tt options("cat\_prefix")}, either a string or a function:
...
You can shut down message by {\tt options("cat\_verbose" = FALSE)}. 
...
You can use it together with {\tt GetoptLong} when you want to run the R script in command line.
...
In above exmaple, {\tt @\{\}} is used to mark variables. Later variables will be extracted from these marks
...
So, for example, you are using {\tt @\textbackslash\textbackslash[CODE\textbackslash\textbackslash]} and your string is {\tt "@[a[1]]"}, it will fail to
extract the correct variable name while only returns {\tt a[1}. In such condition, you should use other styles
...
Since {\tt qq} takes value of the last variable as final return values, you need 
to explicitly specify the return value of the chunk.

Following example is another way to contruct a HTML table in which eval rows and 
odd rows are assigned with different class names.
...
It is important to collapse {\tt text} into one string, or else other part out of the template
codes will be recycled.
