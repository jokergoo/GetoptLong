...
as [**getopt**](https://cran.r-project.org/web/packages/getopt/index.html),
[**optparse**](https://cran.r-project.org/web/packages/optparse/),
[**argparse**](https://cran.r-project.org/web/packages/argparse/),
[**docopt**](https://cran.r-project.org/web/packages/docopt/index.html). Here
...
You can find the short option names (in single characters) are automatically added.
...
specification that contains all types of options (with long description):
...
flexibilities for the usage message. _E.g._ to group options. Here option
specifications are maked by `<>` where the specifications will be extracted
and the text will be replaced by the option description. The mark can be set
by `GetoptLong.options$template_tag` global option.
...
Usage: Rscript foo.R [options]
  ...
  <count=i{2,}> This is a count. This is a count. This is a count. This is a count.
  <#count> This is a count. This is a count.
  <number=f> This is a number. This is a number. This is a number. This is a number.
  <#number> This is a number. This is a number.
", template_control = list(opt_width = c(count = 22, number = 22))
)
```
...
`GetoptLong:::source()`, so it would be convinient to load external R scripts
