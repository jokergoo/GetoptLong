...
\hline
\end{tabular}
\caption{Detailed example of options specification (continued)}
\label{table:spec2}
\end{table}
...
If you don't like such importing, you can set {\tt export} to {\tt FALSE} as:

<<eval = FALSE, echo = TRUE>>==
opt = GetoptLong(spec, export = FALSE)
@

Then options will be returned to {\tt opt} as a list, and there will be no check
for mandatory options and default values.
