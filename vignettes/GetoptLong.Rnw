...
\section{Set command line arguments within {\tt source}}

You can add command line arguments when sourcing an R script so that you can control the options
even when you are in an R interactive session.
