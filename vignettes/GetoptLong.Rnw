...
\hline
{\tt tag=s} & {\tt -}{\tt -tag 1} & {\tt "1"}. Here double quote is used because it is a string. \\
			& {\tt -}{\tt -tag 0.1} & {\tt "0.1"} \\
			& {\tt -}{\tt -tag a} & {\tt "a"} \\
\hline
...
So of course, these already-defined variables should be simple vectors.
...
By default, {\tt GetoptLong} only provides descriptions of all specified options. Users can set
