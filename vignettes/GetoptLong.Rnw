...
Specify any one of alternative option name is OK and it doesn't matter whether using one or two slash in
...
Please note ``{\tt :[desttype]}'' is not supported here. We use another way to define
...
\begin{itemize}
	...
	\item{{\tt o}}: extended integer, an octal string (a zero, optionally followed
...
{\tt tag=i} & {\tt -}{\tt -tag 1} & {\tt 1} \\
            & {\tt -}{\tt -tag 1 -}{\tt -tag 2} & {\tt 2} \\
			& {\tt -}{\tt -tag 0.1} & Value "0.1" invalid for option tag (number expected) \\
			& {\tt -}{\tt -tag a} & Value "a" invalid for option tag (number expected) \\
			& {\tt -}{\tt -tag} & Option tag requires an argument \\
			& no argument & tag is mandatory, please specify it \\
\hline
{\tt tag=s} & {\tt -}{\tt -tag 1} & {\tt "1"} \\
			& {\tt -}{\tt -tag 0.1} & {\tt "0.1"} \\
			& {\tt -}{\tt -tag a} & {\tt "a"} \\
\hline
{\tt tag=f} & {\tt -}{\tt -tag 1} & {\tt 1} \\
			& {\tt -}{\tt -tag 0.1} & {\tt 0.1} \\
			& {\tt -}{\tt -tag a} & Value "a" invalid for option tag (real number expected) \\
\hline
{\tt tag=o} & {\tt -}{\tt -tag 1} & {\tt 1} \\
			& {\tt -}{\tt -tag 0b001001} & {\tt 9} \\
			& {\tt -}{\tt -tag 0721} & {\tt 465} \\
			& {\tt -}{\tt -tag 0xaf2} & {\tt 2802} \\
			& {\tt -}{\tt -tag 0.1} & Value "0.1" invalid for option tag (extended number expected) \\
			& {\tt -}{\tt -tag a} & Value "a" invalid for option tag (extended number expected) \\
\hline
...
\hline
{\tt tag=i@} & {\tt -}{\tt -tag 1} & {\tt 1} \\
            & {\tt -}{\tt -tag 1 -}{\tt -tag 2} & {\tt 1 2} \\
\hline
{\tt tag=i\%} & {\tt -}{\tt -tag 1} & Option tag, key "1", requires a value \\
            & {\tt -}{\tt -tag name=1} & {\tt tag["name"] = 1} \\
\hline
{\tt tag=i\{2\}} & {\tt -}{\tt -tag 1} & Insufficient arguments for option tag \\
            & {\tt -}{\tt -tag 1 2} & {\tt 1 2} \\
            & {\tt -}{\tt -tag 1 -}{\tt -tag 2} & Value "--tag" invalid for option tag \\
\hline
\end{tabular}
...
In some conditions that path of binary perl is not in your {\tt PATH} environment variable and you do not
have permission to modify {\tt PATH}. You can specify your perl path from command line like:
...
take the first argument next to {\tt -}{\tt -} as the path of user-specified perl path.
