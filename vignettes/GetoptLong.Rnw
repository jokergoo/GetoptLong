...
or {\it Python}-style {\tt optparse}. Here {\bf GetoptLong} is another command-line argument
parser which wraps the powerful {\it Perl} module {\tt Getopt::Long} (\url{http://search.cpan.org/perldoc?Getopt%3A%3ALong}),
also provides some adaptation for easier use in R.

Using {\bf GetoptLong} is simple especially for {\it Perl} users because the specification is almost the same as in {\it Perl}.
The original website of \href{http://search.cpan.org/perldoc?Getopt%3A%3ALong}{{\tt Getopt::Long}}
is always your best reference.

\section{Workflow of the wrapping}

Figure \ref{fig:workflow} shows how the R package works for parsing the command-line arguments.
...
The number of arguments in {\tt GetoptLong()} should be even number and the specification and description
...
\begin{itemize}
	...
	\item{{\tt =type[desttype][repeat]}}:
		options should have arguments. Only either {\tt desttype} or {\tt repeat} can be used.
\end{itemize}

Please note ``{\tt :[type][desttype]}'' is not supported here (If you don't know what it is, just ignore it). We use another way to define
...
R variables but it is not allowed for {\tt Getopt::Long} module.
...
{\tt head} and {\tt foot} arguments to
...
Please refer to website of {\tt Getopt::Long}(\url{http://search.cpan.org/perldoc?Getopt%3A%3ALong#Configuring_Getopt%3A%3ALong})
...
\section{Specify command-line options within R session}

When in an interactive R session, arguments can be set when calling {\tt GetoptLong:::source()}, so it would be convinient to control
